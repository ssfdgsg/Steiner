...
//
// 边界（M10）：本对账仅在 store 启用时由装配方启动（main.go 以 st != nil
// 为门）。store 未启用时的广播丢失收敛由 cluster 包承担——每实例把观察到的
// 广播写入 Redis 状态快照，leader 周期合并并重播（见 cluster.go 的
// runStateMirror / runResyncLoop），两者互补覆盖同一缺陷的两个配置面。
//
// 收敛规则：
